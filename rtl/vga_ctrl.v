/********************************************/
/* vga_ctrl.v                               */
/* Generates VGA sync signals & counters    */
/*                                          */
...
module vga_ctrl #(
  ...
  parameter VCW         = 10,       // vertical counter width
  ...
  parameter V_BACK      = 30+2,     // back porch width in lines + added 2 lines for active 'border'
  parameter V_ACTIVE    = 480,      // active time width in lines, acutal width is 484px with border
  parameter V_FRONT     = 9+2,      // front porch width in lines + added 2 lines for active 'border'
...
always @ (posedge clk, posedge rst) begin
  if (rst) begin
    ...
    active <= #1   (h_cnt_r < H_ACTIVE) && (v_cnt_r < V_ACTIVE);
...
always @ (posedge clk, posedge rst) begin
  if (rst) begin
    ...
    a_start = #1 (h_cnt_r == 'd0) && (v_cnt_r == 'd0);
...
always @ (posedge clk, posedge rst) begin
  if (rst) begin
    ...
    h_sync <= #1 ((h_cnt_r >= (H_ACTIVE+H_FRONT)) && (h_cnt_r < (H_ACTIVE+H_FRONT+H_SYNC))) ^ H_POL;
    v_sync <= #1 ((v_cnt_r >= (V_ACTIVE+V_FRONT)) && (v_cnt_r < (V_ACTIVE+V_FRONT+V_SYNC))) ^ V_POL;
  end
end
