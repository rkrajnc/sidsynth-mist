...
// SIDsynth: clamp to 18 bits, for the widened (+2 fractional bits) integrators.
function signed [17:0] clamp18(input signed [18:0] x);
	clamp18 = ^x[18:17] ? {x[18], {17{x[17]}}} : x[17:0];
endfunction

// SIDsynth addition (Tier 3 option 1): 6581 mixer op-amp soft-knee
// compression, centered at the tap DC operating point. Unity below
// COMP_KNEE, gain COMP_HG/16 above. The LP tap carries a large DC offset
// (DC_LP, measured); BP/HP block DC so center at 0. 8580 is left linear.
// Drop this + its use in state 5 for an upstream re-sync.
...
// o = c +- (a * b)
// SIDsynth: integrator state widened 16->18b (+2 frac bits); b/m/o/c grow to
// suit, so the multiply is 16x18 -- still one native Cyclone III 18x18 DSP.
reg signed  [33:0] c;
reg                s;
reg signed  [15:0] a;
reg signed  [17:0] b;
...
// vbp = vbp - w0*vhp
// vhp = 1/Q*vbp - vlp - vi
// SIDsynth: 18-bit (2 extra fractional bits) cuts low-cutoff LP quant noise.
reg signed [17:0] vlp, vlp2, vlp_next;
reg signed [17:0] vbp, vbp2, vbp_next;
reg signed [17:0] vhp, vhp2, vhp_next;
reg signed [18:0] dv;

// SIDsynth: filtered-tap mix and its compressed (6581 mixer op-amp) form.
// tmix_s / center_s are registered snapshots: the compressor runs on these in
// state 6, OUT of the state-4->5 SVF MAC cycle (pipelined to close timing).
...
always_comb begin
	...
	// Narrow the 18-bit taps back to 16-bit signal scale (>>2) so the mixer /
	// compressor / audio path downstream stays at the original scale.
	...
	// Compress the REGISTERED snapshot so compress() is pipelined out of the
	// SVF MAC cycle (used in state 6; closes the slow-corner timing path).
	tmix_c = mode ? clamp(tmix_s)
	              : clamp(17'(compress(clamp(tmix_s - center_s))) + center_s);
end


// Filter -- hoisted from always-local declarations to module scope so they
// retain their value across clock edges under Verilator (which treats
// always-local `reg ... = init;` as automatic, resetting to 0 each entry).
reg [10:0]        _1_Q_lsl10;
reg signed [17:0] vi;   // SIDsynth: wide-scale input (signal<<2) for 18-bit state
reg signed [15:0] vd;
...
always @(posedge clk) begin
	case (state)
		2:	begin
				...
				// SIDsynth: >>>5 (was >>>7) carries vi at the wide 18-bit scale
				// (signal<<2) to match the widened integrator state.
		...
		4:	begin
				...
				// vhp = 1/Q*vbp - vlp - vi  (vlp2/vi both at wide 18-bit scale)
				c <= -(34'(vlp2) + 34'(vi)) << 10;
				s <= 0;
				a <= _1_Q_lsl10; // 1/Q << 10
				b <= vbp_next;   // vbp
		...
		5: begin
				...
				// SIDsynth: snapshot the raw tap mix + center. compress() runs
				// on these next cycle (state 6), out of this SVF MAC cycle.
				tmix_s   <= tmix;
				center_s <= center;
			end
		6: begin
				// SIDsynth: register the compressed mix. compress() runs THIS
				// cycle, isolated from both the SVF MAC (state 4->5) and the
				// audio MAC (state 7) -- splits the long path to close timing.
				tmix_c_r <= tmix_c;
			end
		7: begin
				// SIDsynth: audio MAC. aout = vol*amix; faudio ready at state 8.
				// In the real SID the signal is inverted in the mixer op-amp,
				// then again in the volume control op-amp.
				c <= 0;
				s <= 0;
